 ...
 *
 * SCOPE: GrownBy currently integrates with LiteFarm's dev/beta environment only.
 * Production requires a separate set of GrownBy credentials that do not exist
 * yet; do not insert the production `market_directory_partner_auth` row until
 * they do (the GrownBy tile is data-driven, so it will surface wherever this
 * migration runs even before that row exists).
 *
 * NOTE: the credentials row in `market_directory_partner_auth`
 * (client_id, keycloak_url, keycloak_realm, webhook_endpoint) is intentionally
 * NOT inserted here — it holds secrets and is added out-of-band per environment,
 * matching how the OFN auth row is handled. Contact the PR author for the values.
 *
